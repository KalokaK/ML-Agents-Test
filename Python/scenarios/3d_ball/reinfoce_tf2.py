...
class BehaviourGroup:
    ...
    def __init__(self,
        ...
        if parameters["LOAD_FROM_FILE"]:
            ...
            self.network = K.models.load_model(f'modelbackups/{data["time"]}_model.model')
            self.training_iteration = data["train_it"]
            self.id = data["behav_id"]
        else:
            self.network = self.get_network(self.observation_shapes, self.action_size)
    ...
    def get_network(self, inputs, outputs):
    ...
    def _train_network(self, obs: tf.Tensor, act: tf.Tensor, rew: tf.Tensor):
        ...
        with tf.GradientTape() as tape:
            mu = self.network(obs)  # has shape [horizon, action_size]
            # Obtain pdf of Gaussian distribution
            pdf_value = tf.cast(tf.exp(-0.5 * ((act - mu) / self.sigma) ** 2), tf.float32) * \
                        (1 / (self.sigma * tf.sqrt(tau)))
    ...
    def save_network(self):
        now = time.time()
        self.network.save(f'modelbackups/{datetime.utcfromtimestamp(now)}_model.model')
    ...
    def get_action(self, obs: List[np.ndarray]) -> tf.Tensor:
        ...
        action: tf.Tensor = tf.random.normal(mu.shape, mu, self.sigma)

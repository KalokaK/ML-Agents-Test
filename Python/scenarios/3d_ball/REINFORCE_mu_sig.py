...
parameters = {
    ...
    "BIAS_SIG": 0.54,  # yields sig = 1 with softplus activation
    "DEBUG": False,
    "CLIP": 2.0
}
...
env = UnityEnvironment(file_name='unity/3d_ball.x86_64', worker_id=1, seed=1, side_channels=[], no_graphics=False)
env.reset()
behaviour_names = list(env.behavior_specs)

if parameters["LOAD_FROM_FILE"]:
    latest = 0.0
    for file in os.listdir('logs/training_info'):
        current = datetime.strptime(file, "%Y-%m-%d %H:%M:%S.%f").timestamp()
        latest = current if current > latest else latest
    log_writer = tf.summary.create_file_writer(f'logs/training_info/{str(datetime.fromtimestamp(latest))}')
else:
    log_writer = tf.summary.create_file_writer(f'logs/training_info/{datetime.utcnow()}')
optimizer = tf.optimizers.Adam(lr=parameters["LEARN_RATE"])
...
class BehaviourGroup:
    ...
    def __init__(self,
        ...
        if parameters["LOAD_FROM_FILE"]:
            ...
            with open(f'modelbackups/{latest}_training_params.json', 'r') as fp:
                data = json.load(fp)
            self.network = K.models.load_model(f'modelbackups/{data["time"]}_SIGMU_model.model')
            self.baseline_net = K.models.load_model(f'modelbackups/{data["time"]}_baseline_model.model')
    ...
    def get_network(self, input_shapes, output_shape) -> K.Model:
        ...
        dense_1 = K.layers.Dense(32, tf.nn.relu,  name='den_2')(input_shapes)
        dense_2 = K.layers.Dense(32, tf.nn.relu,  name='den_1')(dense_1)
        mu = K.layers.Dense(output_shape, name='mu',
                            bias_initializer=K.initializers.constant(parameters["BIAS_MU"]))(dense_2)
        sig = K.layers.Dense(output_shape, name='sig_0',
                             bias_initializer=K.initializers.constant(parameters["BIAS_MU"]))(dense_2)
        sig = tf.exp(sig, name='sig_1')
        return K.Model(inputs=input_shapes, outputs=[mu, sig])
    ...
    def _train_network(self, obs: tf.Tensor, act: tf.Tensor, rew: tf.Tensor):
        ...
        """
        creates matrix of this type with shape [horizon, horizon]
        [[ 0,  1,  2]
         [-1,  0,  2]
         [-2, -1,  0]]
        """
        ...
        with tf.GradientTape() as tape:
            ...
            optimizer.apply_gradients(zip(grads_base, self.baseline_net.trainable_variables))
        ...
        with tf.GradientTape() as tape:
            ...
            grads_pi = [tf.clip_by_norm(g, parameters["CLIP"])
                     for g in grads_pi]
            optimizer.apply_gradients(zip(grads_pi, self.network.trainable_variables))
    ...
    def save_network(self):
        ...
        self.network.save(f'modelbackups/{utc_now}_SIGMU_model.model')
        self.baseline_net.save(f'modelbackups/{utc_now}_baseline_model.model')
...
for key in list(env.behavior_specs):
    ...
    behaviour_groups[key] = BehaviourGroup(spec, key, decision_steps)
